...
<title>{{ __('lang.menu') }} - {{ __('lang.modify') }}</title>
</head>

<x-layout>
  <div class="p-5">
    <h1>{{ __('lang.menu') }} - {{ __('lang.modify') }}</h1>
    <div class="d-flex h-full justify-content-end rounded bg-white p-5 mt-5 d-flex flex-column">
      ...
      <form action="{{ route('menu.update', ['id' => $menu->_id]) }}" method="POST">
        ...
        <div class="form-outline mt-3">
          <input type="text" class="form-control form-control-lg" placeholder="{{ __('lang.menuName') }}" name="name" autofocus value={{
            $menu->name }}
          />
        </div>
        @error('name')
        ...
        <div class="d-flex justify-content-center mt-3">
          ...
          <a class="btn btn-light" href="/menu">{{ __('lang.back') }}</a>
        </div>
      </form>
    </div>
  </div>
